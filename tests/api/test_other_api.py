...
@pytest.fixture
def stan_and_bob_accounts(db_session):
...
class TestAccountTransactionTemplate:
    ...
    def test_accounts_transaction_template(self, db_session: Session, stan_and_bob_accounts):
        stan, bob = stan_and_bob_accounts
        ...
        with allure.step("Пытаемся выполнить перевод 200 единиц от Stan к Bob"):
            with pytest.raises(ValueError, match="Недостаточно средств на счете"):
                transfer_money(db_session, stan.user, bob.user, amount=200)
